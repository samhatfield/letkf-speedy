module mod_tmean
    ...
    real(dp) :: save3d  (ix*il,kx,ns3d)     ! 3-D fields saved every post-proc. step
    real(dp) :: save2d_1(ix*il,ns2d_1)        ! 2-D fields saved every post-proc. step
    real(dp) :: save2d_2(ix*il,ns2d_2)        ! 2-D fields saved every step (fluxes)
    real(dp) :: rnsave                      ! post-processing counter

    ! Arrays for the computation of daily-means
    ! (initial./used by DMOUT, updated in TMINC and DMFLUX)
    real(dp) :: save2d_d1(ix*il,ns2d_d1)     ! Daily output saved every post-proc step
    real(dp) :: save2d_d2(ix*il,ns2d_d2)     ! Daily output saved every step (fluxes)
end module
