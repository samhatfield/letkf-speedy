...
subroutine grid(vorm,vorg,kcos)
    use mod_atparam
    use mod_spectral, only: get_scale_fact
    use rp_emulator

    implicit none

    real, intent(in) :: vorm(mx2,nx)
    real, intent(inout) :: vorg(ix,il)
    type(rpe_var) :: vorm_rp(mx2,nx), vorg_rp(ix,il), varm_rp(mx2,il)
    integer, intent(in) :: kcos
    real :: scale_fact

    scale_fact = get_scale_fact(vorm)

    vorm_rp = vorm * scale_fact

    call legendre_inv(vorm_rp,varm_rp)
    call gridx(varm_rp,vorg_rp,kcos)

    vorg = vorg_rp%val/scale_fact
end
!*********************************************************************
subroutine spec(vorg,vorm)
    ...
    type(rpe_var) :: vorg_rp(ix,il), vorm_rp(mx2,nx), varm_rp(mx2,il)
    real :: scale_fact

    scale_fact = get_scale_fact(vorg)

    vorg_rp = vorg * scale_fact

    call specx(vorg_rp,varm_rp)
    call legendre_dir(varm_rp,vorm_rp)

    vorm = vorm_rp%val/scale_fact
end
!*********************************************************************
subroutine vdspec(ug,vg,vorm,divm,kcos)
    ...
    if (kcos.eq.2) then
        do j=1,il
            do i=1,ix
                ug1(i,j)=ug(i,j)*cosgr(j)%val
                vg1(i,j)=vg(i,j)*cosgr(j)%val
            end do
        end do
    else
        do j=1,il
...
subroutine legendre_inv(v_in,v_out)
    ...
    type(rpe_var), intent(in) :: v_in(mx2,nx)
    type(rpe_var), intent(inout) :: v_out(mx2,il)
    type(rpe_var) :: even(mx2),odd(mx2)
...
subroutine legendre_dir(v_in,v_out)
    ...
    ! mx2 = 2*mx because these arrays actually represent complex variables
    type(rpe_var), intent(in) :: v_in(mx2,il)
    type(rpe_var), intent(inout) :: v_out(mx2,nx)
    type(rpe_var) :: even(mx2,iy), odd(mx2,iy)
